...
def rpy_matrix(roll,pitch,yaw):
    ...
    case1 = np.dot(rollMatrix, np.dot(pitchMatrix,yawMatrix))
    return case1
...
def ar_marker_pose_estimation(req):
    ...
    try:
        ...
        for point_i in index_ids:
            ...
            if len(index_ids) == 1:
                pose_msg = pose_arr.poses[0]
                position_distance = 0.34 if pose_msg.orientation.w < 3 or pose_msg.orientation.w > 4 else 0.33
                orientation_matrix = rpy_matrix(pose_msg.orientation.x,pose_msg.orientation.y,pose_msg.orientation.z)
                orientation_x,orientation_y,orientation_z = orientation_matrix[:3,0]

                if pose_msg.orientation.w % 2 == 0: # if id is a even number(front_ar_marker)
                    pose_msg.orientation.w -= 1
                    pose_msg.position.x += position_distance*orientation_x
                    pose_msg.position.y += position_distance*orientation_y
                    pose_msg.position.z += position_distance*orientation_z
                else
                    pose_msg.orientation.w += 1
                    pose_msg.position.x -= position_distance*orientation_x
                    pose_msg.position.y -= position_distance*orientation_y
